...
public class ReceiptController {
	...
	public String submitForm(@ModelAttribute("detail_receipt") List<DetailReceipt> detailReceipts,
...
// Xử lí khi bấm vào nút CTPN -> sẽ lấy dữ liệu ở addPn qua CTPN
	...
	/*
	 * @ModelAttribute("receipts") public List<Receipt> getList_Customer() {
	...
	public String test1(@RequestParam(value = "ids", required = false) List<String> selectedProducts,
			...
			receiptDao.createReceipt(receipt_codeString, supplier,new Date() );
			  for(int i=0;i<selectedProducts.size();i++) {
				...
				/*
				 * List<Receipt> receipts = receiptDao.getListReceipt();
				 * md.addAttribute("receipts",receipts);
				 */
